...
import { Mic, Bell, Users, Eye } from "lucide-react";
import { toast } from "sonner";
...
export default function TeacherDashboard() {
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50">
      <Card className="bg-zinc-950/90 border border-yellow-600/50 shadow-[0_0_30px_rgba(202,138,4,0.2)] p-2 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <div className="px-3 border-r border-zinc-800">
             <span className="text-[10px] font-bold font-mono text-yellow-500 block">ADMIN</span>
             <span className="text-[10px] font-mono text-zinc-500">CONTROLS</span>
          </div>
          <DashboardButton icon={Mic} label="ANNOUNCE" onClick={() => handleAction("Broadcast Started")} />
          <DashboardButton icon={Bell} label="ALARM" onClick={() => handleAction("Class Bell Rung")} />
          <DashboardButton icon={Users} label="SILENCE" onClick={() => handleAction("Global Mute")} />
          <DashboardButton icon={Eye} label="INSPECT" onClick={() => handleAction("Proctor Mode")} />
        </div>
      </Card>
    </div>
  );
...
function DashboardButton({ icon: Icon, label, onClick }: any) {
  return (
    <Button 
      ...
      size="sm" 
      onClick={onClick}
      className="flex flex-col items-center justify-center h-12 w-16 gap-1 hover:bg-yellow-600/20 hover:text-yellow-400 rounded-md transition-all group"
    >
      <Icon className="w-4 h-4 text-zinc-400 group-hover:text-yellow-400" />
      <span className="text-[9px] font-mono text-zinc-500 group-hover:text-yellow-400">{label}</span>
    </Button>
  );
}